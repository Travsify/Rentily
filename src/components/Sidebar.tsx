...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const menuItems = [
    ...
    },
    {
      ...
      icon: BadgePercent,
      badge: `₦${(escrowTotalAmount / 1_000_000).toFixed(1)}M`,
  ...
  return (
    <aside className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col justify-between p-4 min-h-[calc(100vh-65px)]">
      <div className="space-y-6">
        ...
        <div className="p-3.5 rounded-xl bg-gradient-to-br from-emerald-950/60 to-slate-900 border border-emerald-800/40">
          <div className="flex items-center gap-2 text-emerald-400 font-semibold text-xs mb-1">
            ...
            <span>Agent Elimination Model</span>
          </div>
          <p className="text-[11px] text-slate-300 leading-relaxed">
            Rentals: <span className="text-emerald-400 font-bold">10% Legal Fee</span> (Zero Agent 20%).<br/>
            Sales: <span className="text-emerald-400 font-bold">5% Escrow/Search</span> (Zero 10% Agent split).
          </p>
        </div>
      ...
      <div className="p-3 rounded-xl bg-slate-950/80 border border-slate-800/80 text-[11px] text-slate-400 space-y-1">
        ...
        <p className="text-[10px] text-slate-500">Targeting Lagos & Abuja Real Estate Disruption</p>
      </div>
    </aside>
  );
};
