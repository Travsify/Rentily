      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 p-4 rounded-2xl bg-slate-900 border border-slate-800">
        <div className="relative w-full sm:w-96">
          ...
          <input
            ...
            placeholder="Search by title, owner, deed number, or location..."
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {filtered.map((kyp) => {
          ...
          return (
            <div
              key={kyp.id}
              className={`p-5 rounded-2xl bg-slate-900 border transition hover:border-slate-700 flex flex-col justify-between ${
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <span className="text-[10px] font-mono uppercase tracking-wider text-slate-500">
                      {kyp.id} • Submitted {new Date(kyp.submittedAt).toLocaleDateString()}
                    </span>
                    <h3 className="text-sm font-bold text-white mt-0.5">{kyp.propertyTitle}</h3>
                  ...
                  <div>
                    {isPending && (
                      <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-1 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/40 animate-pulse">
                    ...
                    {isRejected && (
                      <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-1 rounded-full bg-red-500/20 text-red-300 border border-red-500/40">
                ...
                <div className="flex items-center justify-between text-xs px-3 py-2 rounded-lg bg-slate-950/40 border border-slate-800/40">
                  <div className="flex items-center gap-1.5 text-slate-300">
                    ...
                    <span>Disco Meter: {kyp.discoProvider} ({kyp.discoMeterNumber})</span>
                  </div>
