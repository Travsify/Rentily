...
export async function checkServerHealth() {
  try {
    ...
    // Return null if local backend server is not running
  }
  return null;
}
...
export class RentillyApiService {
  ...
  static async getProperties(): Promise<Property[]> {
    try {
      const res = await fetch(`${API_BASE}/properties`, { signal: AbortSignal.timeout(1500) });
  ...
  static async getKYPRecords(): Promise<KYPRecord[]> {
    try {
      const res = await fetch(`${API_BASE}/kyp/records`, { signal: AbortSignal.timeout(1500) });
      if (res.ok) return await res.json();
    } catch {}
  ...
  static async getInspections(): Promise<Inspection[]> {
    try {
      const res = await fetch(`${API_BASE}/inspections`, { signal: AbortSignal.timeout(1500) });
      if (res.ok) return await res.json();
    } catch {}
  ...
  static async getTransactions(): Promise<Transaction[]> {
    try {
      const res = await fetch(`${API_BASE}/escrow/transactions`, { signal: AbortSignal.timeout(1500) });
      if (res.ok) return await res.json();
    } catch {}
  ...
  static async getLegalAgreements(): Promise<LegalAgreement[]> {
    try {
      const res = await fetch(`${API_BASE}/legal/agreements`, { signal: AbortSignal.timeout(1500) });
      if (res.ok) return await res.json();
    } catch {}
